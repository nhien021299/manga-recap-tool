...
from app.prompts.effect_planner_prompt import EFFECT_PLANNER_PROMPT_V2
...
- smooth_zoom_fade: Crossfade with subtle zoom (400-600ms).
- hard_cut: Instant cut (0ms).

VFX Tags (vfx_tags): 
- atmospheric: film_grain, dust, cold_mist, rain, fire_embers, dark_smoke.
- dramatic: blood_spatter, speed_lines, edge_glow, color_pulse.
- framing: letterbox.

Color Grading (color_grade):
- warm_firelight: Warm, cozy or firelit scenes.
- cold_blue: Sad, mysterious, or night scenes.
- cold_dusk: Darker, blue-purple mystery.
- dark_jade: Ancient, mystical, or cultivation energy.
- blood_amber: Dangerous, violent, or sunset tension.
- neutral: Default natural look.
""".strip()
...
class VideoDirectorService:
    ...
    def _build_prompt(
        ...
        *,
        ...
        # Build compact scene summary
        import json
        scene_summary = []
        for scene in package.scenes:
            # scene_num, type, mood, short_desc, intensity
            short_desc = f"{scene.title}: {scene.narration[:60]}"
            intensity = "mid"
            if scene.scene_type in ["combat_action", "danger_build", "shock_reveal"]:
                intensity = "high"
            elif scene.scene_type in ["establishing", "emotional_pause"]:
                intensity = "low"
                
            scene_summary.append([
                scene.scene,
                scene.scene_type or "establishing",
                scene.mood or "neutral",
                short_desc,
                intensity
            ])
            
        scene_json = json.dumps(scene_summary, ensure_ascii=False)
        return EFFECT_PLANNER_PROMPT_V2.replace("{scene_json}", scene_json)
    ...
    def _parse_direction(
        ...
        *,
        ...
        try:
            return EffectPlanNormalizer.normalize_plan(
                raw_json=raw_text,
                package=package,
                tts_result=tts_result,
                width=width,
            ...
            logger.error("Failed to normalize video direction JSON: %s", exc)
    ...
    def _build_fallback_direction(
        ...
        *,
        ...
        for index, scene in enumerate(package.scenes):
            ...
            trans_ms = scene.transition_duration_ms if scene.transition_duration_ms else 500
            duration_ms = max(target_ms, audio_ms + trans_ms + 100)
