...
class GeminiScriptService:
    ...
    def _build_unified_prompt(
        ...
        *,
        ...
        mode_bias_map = {
            "horror": "Bias: dread, unease, imminent threat.",
            "combat": "Bias: speed, impact, split-second danger.",
            "escape": "Bias: urgency, pursuit, survival pressure.",
            "investigation": "Bias: clues, suspicion, hidden danger.",
            "aftermath": "Bias: damage, silence, lingering pressure.",
            "mystery": "Bias: curiosity, strange detail, unresolved danger.",
        }
        mode_bias = mode_bias_map.get(narration_mode, mode_bias_map["mystery"])

        sections: list[str] = [
            f"Write final {language_label} manga recap narration for YouTube.",
            "Goal: maximize retention with concise, cinematic narration.",
            f"Mode: {narration_mode}. {mode_bias}",
        ]
...
".join(
                    [
                        "Previous context:",
                        previous_summary,
                        "Keep continuity unless the current images clearly shift scenes.",
                    ]
                )
            )
        if identity_evidence.confirmed_names:
            sections.append(f"Confirmed names in this batch: {', '.join(identity_evidence.confirmed_names)}")
...
".join(
                    [
                        ...
                        "Do not treat carryover names as proof if the current images do not confirm them.",
                    ]
                )
            )
        if identity_evidence.use_neutral_fallback:
            sections.append("Identity confidence is low. Use neutral labels unless this batch clearly reveals a name.")
...
".join(
                [
                    ...
                    "- Do not invent identity, motive, outcome, or twist.",
                    "- Use names only when this batch clearly supports them.",
...
".join(
                [
                    ...
                    '- No greeting, intro, outro, or filler.',
                    "- Make adjacent panels flow as one recap, not isolated captions.",
                    "- Prefer 1 short sentence per panel; use 2 only if the second adds consequence or curiosity.",
                    "- Compress wording by about 10-15% versus a normal recap.",
                    "- If a line only restates the image, cut it or rewrite it.",
                    "- Do not narrate framing, pose, clothing, or object placement unless it changes the story beat.",
                    "- Prefer action, consequence, danger, or curiosity over visual listing.",
                    "- Keep wording clear, punchy, and easy for TTS.",
                    "- Vary rhythm. Not every panel should peak at maximum intensity.",
                ]
            )
        )
        sections.append(
...
".join(
                [
                    "Style guardrails:",
                    "- Avoid generic or repetitive emotional phrasing.",
                    "- Avoid repeating the same visual idea across nearby panels unless the danger clearly escalates.",
                    "- Avoid flat labels when a more specific neutral descriptor is visible.",
                    "- Any Gen Z flavor must stay subtle, brief, and fit the cinematic tone.",
                ]
            )
        )
        sections.append(
...
".join(
                [
                    "Batch ending:",
                    "- Build momentum across the batch.",
                    "- If the final panel suggests unresolved danger, discovery, or confrontation, end with a short natural cliffhanger.",
                    "- The final line should pull the viewer forward, not merely summarize the image.",
                ]
            )
        )
        sections.append(
...
".join(
                [
                    ...
                    "- Every voiceover_text must sound like a compelling recap line, not a dry description.",
